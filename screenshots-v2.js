...
(async () => {
  ...
  // Inject localStorage data before navigation
  ...
  // 1. Full page screenshot (1280x900 viewport)
  await page.setViewportSize({ width: 1280, height: 900 });
  await page.screenshot({ path: `${dir}/full-v2.png`, fullPage: false });
  console.log('full-v2.png saved');

  // 2. Schedule cards screenshot - scroll to show the timeline
  // First scroll to hide the header, showing just the schedule
  await page.evaluate(() => window.scrollTo(0, 200));
  await page.waitForTimeout(500);
  await page.setViewportSize({ width: 1280, height: 800 });
  await page.screenshot({ path: `${dir}/schedule-v2.png`, fullPage: false });
  console.log('schedule-v2.png saved');

  // 3. Status bar screenshot - use a clip region to capture just the bottom 80px
  // Set viewport back to full size first
  await page.setViewportSize({ width: 1280, height: 900 });
  // Scroll to bottom
  await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
  await page.waitForTimeout(500);
  // Get the visible page height
  const pageHeight = await page.evaluate(() => document.documentElement.scrollHeight);
  // Clip from the bottom 80px
  const statusBarY = Math.max(0, pageHeight - 80);
  await page.screenshot({
    path: `${dir}/statusbar-v2.png`,
    fullPage: false,
    clip: { x: 0, y: statusBarY, width: 1280, height: 80 },
  });
  console.log('statusbar-v2.png saved');
