...
(async () => {
  ...
  // Verify em-dash is rendering correctly
  const firstCard = await page.locator('.schedule-card').first();
  const timeText = await firstCard.textContent();
  console.log('First card time text:', JSON.stringify(timeText));

  // Check if em-dash is correct
  if (timeText.includes('–')) {
    console.log('✅ Em-dash rendering correctly!');
  } else if (timeText.includes('u2013')) {
    console.log('❌ Em-dash still broken!');
  }
